...
func (n *Node) listenForMessages() {
	for {
		...
		if msg.messageType == "request" {
			...
			if n.wantToEnterCS && len(n.receivedReplies) == numberOfNodes-1 && n.priorityQueue[0].senderID == n.id {
			...
			if len(n.receivedReplies) == numberOfNodes-1 && n.priorityQueue[0].senderID == n.id {
				// enter critical section
				go n.enterCriticalSection()
			}
		}
